...
class BFS():
    ...
    def __init__(self, debug=True):
        ...
        self.creates = 1           # cantidad de posiciones expandidas
        self.expanded = 0            # cantidad de posiciones creadas
        self.strdebug = ""
    ...
    def start(self,environment, operator):
        ...
        while len(open_pos) > 0:    # mientras halla algo en la cola
            ...
            open_coords.remove(coords)  # Elimino esta posicion
                                        # de la cola
            ...
            if coords in end_coords and current.getParent():
                ...
                if end_coords.index(coords) == current.getTargets().index(False):
                    ...
                    self.strdebug += "target_found>>: "+str(tuple(reversed(\
            ...
            for x,y in operadores.apply_operator(operator,coords):
                ...
                if (x, y) not in obstacles_coords and\
                    ...
                    if self.debug:
                        print "libres: ",open_coords
                    self.creates+=1 # aumento la cantidad de creados


            # para pintar la cola
            arr_str = []
            for x in open_coords:
                arr_str.append( tuple(reversed(x)) )

            self.strdebug += "queue: "+str(arr_str)+"\n"
